package com.ace.Input
{
	...
	public class Utilities
	{
		...
		public static function validateAll(object:Object):Boolean
		{
			...
			{
				if (field is ValidatedTextInput || field is ValidatedDateField || field is VAComboBox ||
				field is ValidatedComboBox || field is ValidatedTextArea || 
				field is ValidatedRadioButtonBox)
				{
					bolTemp = field.validateData();
					if (bolTemp) bolTemp = field.isValid;
					if (bolAllValid) bolAllValid = bolTemp;
				}
//				if (field["id"]) trace('field["id"]=' + field["id"] + "|bolTemp=" + bolTemp + '|bolAllValid=' + bolAllValid);
		...
		public static function setQueryAll(object:Object,objRestrict:Object,queryFields:String):Object
		{
			...
			if (!objRestrict) objRestrict = new Object;
 			for each (var field:Object in object.getChildren())
			{
					...
					field is ValidatedTextArea)
				{
					...
					if (field["charactersAlsoPermitted"].toString().indexOf("_%><=") < 0)field["charactersAlsoPermitted"] += "_%><=";
					if (field["editable"] == false) field["editable"] = true;
					if (field["enabled"] == false) field["enabled"] = true;
					if (!(field is ValidatedTextArea))field["formatData"] = false;
				...
				{
					if (field is ValidatedTextInput || field is ValidatedDateField ||
						field is ValidatedTextArea || field is ValidatedComboBox || 
						field is ValidatedRadioButtonBox)
					{
						if (strQueryFields.indexOf("," + field["id"] + ",") < 0)
						{
							field["enabled"] = false;
							if (field.hasOwnProperty("editable")) field["editable"] = false;
						}
					}
				}
		...
		**/
		...
		public static function clearAll(object:Object):void
		{
			for each (var field:Object in object.getChildren())
			{
				...
				if (field is ValidatedComboBox || field is ValidatedCheckBox || 
					field is ValidatedRadioButtonBox)
				{
					field["value"] = "";
					field["errorString"] = "";
					if (field["enabled"] == false) field["enabled"] = true;
					if (field is ValidatedComboBox) if (field["editable"] == true) field["editable"] = false;
				}
				if (field is Text) field["text"] = "";
				
				if (field is DataGrid ) { field["dataProvider"] = null; }
		...
		public static function setDefaultAll(object:Object):void
		{
			for each (var field:Object in object.getChildren())
			{
				if (field is ValidatedTextInput || field is ValidatedDateField ||
					field is ValidatedTextArea ||field is ValidatedComboBox || 
					field is ValidatedCheckBox || field is ValidatedRadioButtonBox)
				{
					field.setDefault();
		...
		public static function setDisabledAll(object:Object):void
		{
			for each (var field:Object in object.getChildren())
			{
				...
				if (field is ValidatedComboBox || 
					field is ValidatedCheckBox || 
					field is VAComboBox || 
					field is ValidatedRadioButtonBox || field is ValidatedDateField)
				{
					field["enabled"] = false;
				}
				if (isContainer(field))
		...
		private static function isContainer(obj:Object):Boolean
		{
			return (obj is Canvas || obj is FormItem || obj is TabNavigator || obj is HBox || obj is VBox ||
					obj is Panel || obj is TitleWindow || obj is ControlBar || obj is Grid || obj is GridRow || obj is GridItem);
		}
