...
/**
 *  The <code>SelectionDataGrid</code> control is like a TreeDataGrid except that it can 
 ...
 *  </p>
 *  The SelectionDataGrid class is a base Class for ZetDataGrid.
 * 
 *  @includeExample SelectionDataGridExample.mxml
 **/
public class SelectionDataGrid extends TreeDataGrid implements ICellSelectable
{
	 ...
	 * Constructor
	 **/
	public function SelectionDataGrid() {
		super();
		...
		this.addEventListener(DataGridEvent.COLUMN_STRETCH, columnStretchHandler, false, 0, true);
		this.addEventListener(Event.MOUSE_LEAVE, leaveHandler, false, 0, true);
		this.addEventListener(DataGridEvent.HEADER_RELEASE, headerReleaseHandler, false, 0, true);
	}
    ...
    **/
	 ...
	 * @private
	 * 
	 * Internal property to track the first item renderer instance that user has pressed on. This property
	 * is set its value by using mouseToITemRenderer method.
	 **/
	 ...
	 * @private
	 * 
	 * Internal Point instance that saves information on the x and y position of the Point where user
	 * has first pressed on the DataGrid.
	 **/
	 ...
	 * 
	 * Internal Point instance that saves information on the x and y position of the Point where user
	 * has last dragged on the DataGrid.
	 **/
	protected var endPoint:Point;
		/**
	 * @private
	 * 
	 * Internal Object that saves information such as startIndex, endIndex, startColumnIndex, endColumnIndex
	 ...
	 * cells of this DataGrid. Default value is ColumnSeries.
	 **/
	 ...
	 * 
	 * @return An ArrayCollection instance that holds all the data of the drag-selected area. 
	 **/
	 ...
	 **/
	protected function getSelectionInfo():Object {
		...
		for (var i:int = 0; i < listItems.length; i++) {
			for (var j:int = 0; j < visibleColumns.length; j++) {
				...
				if (itemRenderer && hasXY(itemRenderer)) {
					...
					if (itemRenderer.y + this.headerHeight <= point.y && itemRenderer.y + itemRenderer.height + this.headerHeight >= point.y) {
						updateRowIndexes(this.dataProvider.getItemIndex(itemRenderer.data));
					}
					/* if (i == 0 && point.y <= itemRenderer.y) {
						selectionInfo.endIndex = this.verticalScrollPosition;
					} */
					if (i == listItems.length -1 && point.y >= itemRenderer.y + itemRenderer.height + this.headerHeight) {
    		...
    		if (endIndex >= 0) {
    			try {
		    		...
		    		if (startItem && endItem) {
			    		...
			    		var yPos:Number = startItem.y;//+ cachedPaddingTop;
			    		if (yPos + this.headerHeight + cachedPaddingBottom + height  >= this.height) {
			    			height += (this.height - (yPos+ this.headerHeight + height + cachedPaddingBottom));
			    		}
			    		drawingLayer.drawRect(startItem.x, yPos + this.headerHeight,width,height);
			    		//drawingLayer.drawRect(startItem.x, yPos + this.headerHeight + this.cachedPaddingBottom,width,height);
		    		}
    			} catch (e:Error) {
    			}
    		}
	...
	protected function makeChart():void {
		org.openzet.utils.ChartUtil.showPopUpChart(seriesType, dragSelectionData, xField);
	}
	...
	protected function drawSelection():void {
		...
		if (Application.application.mouseY > pt.y + this.headerHeight && isPressed) {
        ...
        g.clear();
   		...
   		if (item) 
   		{
   			if (enableDragSelection) 
   			{
	   			...
	   			g.beginFill(this.getStyle("dragSelectionColor"),this.getStyle("dragSelectionAlpha"));
	   			g.lineStyle(this.getStyle("dragLineThickness"),0x000000);
	   			var yPos:Number = item.y;// + cachedPaddingTop;
	   			var tempHeight:Number = item.height;
	   			//drawingLayer.drawRect(item.x, yPos + this.headerHeight + this.cachedPaddingBottom, item.width, tempHeight);
	   			drawingLayer.drawRect(item.x, yPos + this.headerHeight, item.width, tempHeight);
	 ...
	 * @private
	 * 
	 * headerRelease event handler to reset dragSelection. 
	 */
	 ...
	 * @private
	 * 
	 * Internal mouseMove event handler for a sprite used as a drag selection layer.
	 **/
		...
		if (allowDragMove) {
    		...
    		if (endIndex >= 0) {
	    		...
	    		if (startItem && endItem) {
		    		...
		    		img.addChild(dr);
					DragManager.doDrag(this,dragSource,event,img as IFlexDisplayObject, - startItem.x, - startItem.y - this.headerHeight);
	    		}
    		}
		}
	}
